...
//
double const Icnst_EC3= 0; //check point -0.1 -> 0.1  after change ahp 0.1 ->-0.1->0.0
double const Gec2     = 0.4;  //[uS] 
double const Gn_EC3_E = 0.02; //check point 0.035 -> 0.025 ->0.03 ->0.02
double const Gn_EC3_I = 0.015; // 0.025 -> 0.015
double const Gn_EC3_IE = 0.005;  
double const Gn_EC3_II = 0.02; 
double const rec3     = 35; //[Hz] //check point 30 -> 35  after change ahp 35->30->35

// within EC5
double GIE_EC5 = 0.5;  //check point 0.2->0.5
double GEI_EC5 = 1.0;  //check point 0.5 ->0.8->1.3 ->2.0
...
#define GIE_EC5_EC3 0.1
#define cIE_EC5_EC3 0.3 

double const Asup        = 0.1;  // effect by OLM
...
double const Gms_ec5   =  0.1;
double const rec5     = 30;  

// within CA1
double const GEE_CA1 = 0;   // Gpost,pre
double const GEI_CA1 = 2.2;  //check point 2.2 
...
double const GEE_EC5_EC3 = 0.01; //check point0.007->0.015->0.013  after ahp changed , 0.013 -> 0.01->0.008
double GEE_CA1_EC5 = 0.0025;  
double const cEE_CA1_EC5 = 0.005;
#define GIE_CA1_EC5 0.01 
#define cIE_CA1_EC5 0.3 
double const G_Caspk = 0.01; //check point 0.04->0.02 ->0.01
...
double dEmin_btw = 10.0;  // check point 5.0 -> 10.0 in gamma_dec.cpp
double dImax_btw = 15.0;
double dImin_btw = 5.0;
...
void calc(int ik,int it,double c_Ach_aft, double Ach_test){
  ...
  */
  ...
  double ca_dyn[Nhst_ca];
  for(int i=0; i<Nhst_ca;i++){
    ca_dyn[i]=exp(-i*hstp/tdcy_ca);
  }
  ...
  for(t = 0.0; t < T+hstp; t += hstp){
    ...
    for(int i = 0; i < NE_CA1; i++){
      ...
      if(pstr_var_CA1->if_spk[i]==1 && pstr_var_CA1->ec3spts[i][2]-pstr_var_CA1->ec3spts[i][0]<Tampec3){
