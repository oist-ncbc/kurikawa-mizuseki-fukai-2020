...
#2018.7.3
def get_rate(data,args):
    (icell_s,icell_e)=args[0]
    T      =args[1]
    inv_bin=args[2]

    rate=np.zeros((icell_e-icell_s,T+1))
    
    for i in range(len(data[:,0])):
        if(data[i,1]<icell_e and data[i,1]>=icell_s):
            rate[int(data[i,1])-icell_s,int(data[i,0]*inv_bin)]+=inv_bin*1000.0  # # of spikes per 1s, 1cell
...
def get_rate_EC(data,args):
    ...
    for i in range(len(data[:,0])):
        if(data[i,1]<ncell):
            rate[int(data[i,1]),int(data[i,0]*inv_bin)]+=inv_bin*1000.0  # # of spikes per 1s, 1cell
